...
public class MessageDatabaseTest {
    ...
    public void testMessageDatabase() throws BadDataException {
        ...
        try {
            ...
            Assert.assertEquals("Make sure BadDataException is thrown by sendMessage()",
                BadDataException.class, e.getClass());
        }
    }
// if this test case is not working it may be because the files for the users is not created yet
    ...
    // the next two tests are relatively wholistic, they cover the reading and writing of files and updating after
    ...
    public void messageDeletionTest() throws IOException {
        ...
        try (BufferedReader send = new BufferedReader(new FileReader("user1.txt"));
            ...
            assertEquals("make sure messages are acutally being deleted from files", sent,
                otherSentMessage.toString());
            assertEquals("make sure messages are acutally being deleted from files", recieved,
                otherSentMessage.toString());
        } catch (IOException e) {
